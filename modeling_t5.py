...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config):
        ...
        self.t5_proj = nn.Linear(1024, 512)
        #self.conv1d = nn.Conv1d(in_channels=768, out_channels=768, kernel_size=2, stride=2, padding=0)
    ...
    def forward(
        ...
        if encoder_outputs is None:
            ...
            hidden_states = self.t5_proj(torch.concat([encoder_outputs[0],criteria_encoder_outputs[0]],dim=1).permute(0,2,1)).permute(0,2,1)
            #hidden_states = self.conv1d(torch.concat([encoder_outputs[0],criteria_encoder_outputs[0]],dim=1).permute(0,2,1)).permute(0,2,1)
